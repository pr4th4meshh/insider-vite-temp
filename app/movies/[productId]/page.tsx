...
const ProductDetail = ({params}: {params: any}) => {
  ...
  const { data: product, isLoading: productLoading, isError:productError  } = useProductQuery(params.productId);
  const { data: products, isLoading: productsLoading, isError:productsError, isSuccess: productsSuccess } = useProductsQuery();

  // console.log(params)

  if (productLoading) return <div><LoadingPage/></div>;
  if (productError) return <div>Error while fetching data</div>;

  return (
    <>
     <div className={`${style.main_div} xxs:text-black sm:text-white xxs:py-[10px] sm:py-[40px] sm:csz flex sm:flex-row xxs:flex-col bg-center `} style={{backgroundImage: `url(${product?.backgroundImg})`}} >
     <div className="flex flex-1 flex-col z-[2] justify-center backdrop-blur-[2px] ">
    <div>
    <button className=" xxs:text-white cursor-pointer xxs:text-[16px] sm:text-[15px] text-center border px-2 py-[5px] mb-[10px] rounded-xl bg-black bg-opacity-50  " onClick={handleClick} > &larr; Previous</button> 
    </div>
       {/* <Image
         src={product?.titleImg}
         height={100}
         width={300}
         className="sm:h-[200px] xxs:h-[250px]"
         alt={""}
       /> */}
       <div>
       <h1 className=" text-pink-700 pb-2 xxs:text-[32px] sm:text-[42px] lg:text-[54px] rounded-2xl "> {product?.title} </h1>
       <h1 className="flex flex-row items-center rounded-2xl xxs:text-[32px] sm:text-[42px] lg:text-[32px]">
         <FaHeart style={{ color: "red", marginRight: "10px" }} /> {product?.rating} / 5
       </h1>
       ...
       <h1 className="xxs:pb-1 sm:pb-2 xxs:text-[32px] sm:text-[42px] lg:text-[28px] rounded-2xl">2D,ICE 3D,IMAX 3D,4DX 3D ,3D</h1>
       <h1 className="xxs:pb-1 sm:pb-2 xxs:text-[32px] sm:text-[42px] lg:text-[28px]  rounded-2xl">Languages: Hindi</h1>
     ...
     <div className="flex flex-1 flex-col xxs:pt-[28px] sm:pt-[100px] xxs:pl-0 sm:pl-[60px] xxs:text-lg sm:text-xl z-[2] xxs:flex-wrap ">
         {/* <h1 className=" text-pink-700 pb-4 xxs:text-[32px] sm:text-[42px]   rounded-2xl "> {product?.title} </h1>
       <h1 className="flex flex-row items-center   rounded-2xl">
         <FaHeart style={{ color: "red", marginRight: "10px" }} /> {product?.rating} / 5
       </h1>
       <h1 className="text-pink-700 max-w-max xxs:pb-2 sm:b-4 cursor-pointer   rounded-2xl ">
         Add your reviews and ratings..
       </h1>
       <h1 className="xxs:pb-1 sm:pb-2   rounded-2xl">2D,ICE 3D,IMAX 3D,4DX 3D ,3D</h1>
       <h1 className="xxs:pb-1 sm:pb-2   rounded-2xl">Languages: Hindi</h1>
       <h1 className="xxs:pb-1 sm:pb-4   rounded-2xl">
        {product?.subTitle}
       </h1>
       <button className="xxs:p-2 sm:p-2 bg-pink-700 sm:max-w-max text-white rounded-lg mb-4 ">
         Book Tickets
       </button> */}
     </div>
   </div>
  ...
  <div className="overflow-hidden max-w-[170px] h-auto max-h-max mb-[10px] cursor-pointer  ">    
   ...
   <Image src={artist.img} className='lg:h-[140px] lg:w-[140px] sm:h-[130px] sm:w-[130px] xxs:h-[90px] xxs:w-[90px] mb-2 rounded-full' alt="" />
     ...
     <Slider {...settingsB}>
        {productsLoading  && <h1 className="text-center text-2xl" >Loading..</h1> }
     {productsError && <h1>Something went wrong</h1> }
        {productsSuccess && products.slice(0,7).map((product) => (
          <Card
          key={product?.id}
          id={product?.id}
